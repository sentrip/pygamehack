...
def string_to_classes(src: str) -> ['Class']:
    """
    Convert a string into a map of class names to Class definitions
    """
...
def classes_to_string(
    ...
    """
    Convert a map of class names to Class definitions into a string (the source required to use these classes)
    """
...
def replace_class_fields(
    ...
    using the give replacer to replace the fields
    """
...
def classes_to_tuples(classes: ['Class']) -> [(str, Any)]:
    """
    Convert a map of class names to Class definitions into a flat list of tuples of the fields
...
def tuples_to_classes(tuples: [(str, Any)]) -> ['Class']:
    """
    Convert a flat list of tuples into a map of class names to Class definitions
...
class AbstractCodeGenerator(AbstractCodeManipulator):
    ...
    def field_to_string(self, field: 'Field') -> str:
        return f'0x{field.data:X}' if self.write_int_as_hex and isinstance(field.data, int) else str(field.data)
...
class DefaultPythonCodeGenerator(AbstractCodeGenerator):
    ...
    def write_field(self, field: 'Field'):
        ...
        self.write(fmt.format(field.name, self.field_to_string(field), inline_comment_src))
...
class DefaultPythonCodeReplacer(AbstractCodeReplacer):
    ...
    def regenerate(self, field: 'Field', previous_lines: [str]) -> str:
        ...
        if field.annotation_src:
            fmt = '{:%ds}: {:%ds}= {}{}' % (self.min_field_name_length, self.min_annotation_length)
            args = field.name, field.annotation_src, self.field_to_string(field), inline_comment_src
        else:
            fmt = '{:%ds}= {}{}' % self.min_field_name_length
            args = field.name, self.field_to_string(field), inline_comment_src

        return field.indent_str + fmt.format(*args)
